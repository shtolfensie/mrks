...
class RemindersAddForm extends Component {
  constructor(props) {
    super(props);
  ...
  shortcuts = {
    remindersAddOpen: ['r e m']
  }

  componentDidMount() {
    Mousetrap.bind(this.shortcuts.remindersAddOpen, () => {
      this.handleOpen();
    });
  }

  componentWillUnmount() {
    Mousetrap.unbind(this.shortcuts.remindersAddOpen);
  }
  ...
  handleAdd = (user) => {
    ...
    if (name === '' || subjectId === '' || dueDate === '' ) {
      ...
      const reminderId = db.ref(`marks-app/${user.uid}/reminders`).push(reminder).key;
      db.ref(`marks-app/${user.uid}/subjects/${reminder.subjectId}/reminderIds`).push({ reminderId });
  ...
  render() {
    ...
    return (
      ...
      { user => (
        <Modal
          trigger={ <Menu.Item onClick={this.handleOpen}>Add a Reminder</Menu.Item> }
          open={open}
...
export default RemindersAddForm;
