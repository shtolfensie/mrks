...
class HomeworkAddForm extends Component {
  constructor(props) {
    super(props);
  ...
  shortcuts = {
    homeworkAddOpen: ['h o m e']
  }

  componentDidMount() {
    Mousetrap.bind(this.shortcuts.homeworkAddOpen, () => {
      this.handleOpen();
    });
  }

  componentWillUnmount() {
    Mousetrap.unbind(this.shortcuts.homeworkAddOpen);
  }
  ...
  handleOpen = ()  => {
    ...
    if (this.props.subjects[0] === false) {
      ...
      alert("Looks like you don't have any subjects yet. Please add at least one subject before adding any homework.");
      this.handleClose();
    }
    else {
  ...
  handleAdd = (user) => {
    ...
    if (name === '' || subjectId === '' || dueDate === '' ) {
      ...
      const homeworkId = db.ref(`marks-app/${user.uid}/homework`).push(homework).key;
      db.ref(`marks-app/${user.uid}/subjects/${homework.subjectId}/homeworkIds`).push({ homeworkId });
  ...
  render() {
    ...
    return (
      ...
      { user => (
        <Modal
          trigger={ <Menu.Item onClick={this.handleOpen}>Add Homework</Menu.Item> }
          open={open}
          ...
          style={{ zIndex: '999' }}
        >
          <Modal.Header>Add Homework</Modal.Header>
          <Modal.Content>
