...
class RemindersEditForm extends Component {
  constructor(props) {
    super(props);
  ...
  handleSave = (user) => {
    ...
    if (name === '' || subjectId === '' || dueDate === '' ) {
      ...
      db.ref(`marks-app/${user.uid}/reminders/${this.props.reminder.key}`).update(reminder);
      if (subjectId !== this.props.reminder.subjectId) {
        db.ref(`marks-app/${user.uid}/subjects/${subjectId}/reminderIds`).push({ reminderId: this.props.reminder.key });
        db.ref(`marks-app/${user.uid}/subjects/${this.props.reminder.subjectId}/reminderIds`).once('value', reminderIdKeys => {
          reminderIdKeys.forEach(reminderIdKey => {
            if (this.props.reminder.key === reminderIdKey.val().reminderId) db.ref(`marks-app/${user.uid}/subjects/${this.props.reminder.subjectId}/reminderIds/${reminderIdKey.key}`).remove();
          })
        })
      }
...
export default RemindersEditForm;
